...
- **It is canonical Rescue-Prime (Rescue-XLIX).** Both half-rounds apply the MDS after the S-box,
  differing only in the S-box direction: forward `S-box -> MDS -> +c`, backward
  `S-box⁻¹ -> MDS -> +c`. The lane-mixing layer is therefore `MDS` (verified MDS by
  `mds_matrix_is_mds`), and published Rescue-Prime cryptanalysis applies to this construction with
  these parameters (Goldilocks, width 12, 7 rounds, α=7). An earlier revision used a non-standard
  variant whose backward half-round applied the MDS before the inverse S-box (making the effective
  linear layer `MDS²`); it was replaced in v0.7.0. Note that these specific round constants and MDS
  matrix have still not had a dedicated third-party review.
