...
mod tests {
    ...
    fn test_mds_inverse_property() {
        ...
        let recovered = mds_inv_multiply(&mixed);

        for i in 0..STATE_WIDTH {
            assert_eq!(
                felt_to_u64(state[i]),
                felt_to_u64(recovered[i]),
                "MDS inverse should recover original state at index {}",
                i
            );
        }
    }
    ...
    fn test_state_width() {
        ...
        assert_eq!(
            RATE + CAPACITY,
            STATE_WIDTH,
            "Rate + Capacity should equal State Width"
        );
    }

    #[test]
    fn test_rescue_constants_hash() {
        // Hash all Rescue constants to detect accidental changes.
        // Serialization is row-major with u64 values in little-endian.
        ...
        // If this changes, update with intent and document the reason.
        assert_eq!(
            digest_hex, "2936f26121c35c83d3a1922855d289ac0e6b6be4e31874cc13233239bc3adb5b",
