...
pub const AGENT_AUTHORIZATION_PROOF_BUNDLE_VERSION: u32 = 1;
...
///
/// This bundles proof bytes, proof metadata, receipt-bound public inputs, and the
/// delegated commerce receipt into a single locally verifiable artifact.
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct AgentAuthorizationProofBundle {
    ...
    pub witness_commitment_hex: Option<String>,
    /// Canonical receipt-bound public inputs.
...
impl AgentAuthorizationProofBundle {
    /// Create a canonical authorization proof bundle from a generated proof, public inputs, and
    ...
    pub fn new(
        ...
        let bound_public_inputs = public_inputs
            .bind_authorization_receipt(&normalized_receipt)
    ...
    pub fn verify(&self) -> Result<VerificationResult> {
        self.validate()?;
        let proof_bytes = self.proof_bytes()?;
        ves_stark_verifier::verify_agent_authorization_proof_auto_bound(
            &proof_bytes,
            &self.public_inputs,
            &self.receipt,
        )
    ...
    pub fn verify_strict(&self) -> Result<VerificationResult> {
        self.validate()?;
        let proof_bytes = self.proof_bytes()?;
        ves_stark_verifier::verify_agent_authorization_proof_auto_bound_strict(
            &proof_bytes,
            &self.public_inputs,
            &self.receipt,
        )
...
impl ProofSubmission {
    ...
    /// Attach receipt-bound canonical public inputs to an agent authorization submission.
...
mod tests {
    use super::*;
    use ves_stark_primitives::public_inputs::{compute_policy_hash, CompliancePublicInputs};
    ...
    fn sample_authorization_bundle(max_total: u64) -> AgentAuthorizationProofBundle {
        ...
        let bound_inputs = inputs.bind_authorization_receipt(&receipt).unwrap();
        ...
        let proof = ComplianceProver::with_policy(policy)
            .prove(&witness)
            .unwrap();
        AgentAuthorizationProofBundle::new(&proof, &inputs, &receipt).unwrap()
    }
    ...
    fn test_agent_authorization_proof_bundle_roundtrip_and_submission() {
        ...
        assert_eq!(submission.policy_id, "agent.authorization.v1");
        assert_eq!(
            submission.public_inputs.unwrap().authorization_receipt_hash,
